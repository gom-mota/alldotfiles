...
active_text_color=$(echo $(xrdb -query | grep *.foreground| awk '{printf "%s", $NF}'))
active_bg=
active_underline=$(echo $(xrdb -query | grep *.accent| awk '{printf "%s", $NF}'))

inactive_text_color=$(echo $(xrdb -query | grep *.foreground| awk '{printf "%s", $NF}'))
inactive_bg=
inactive_underline=
...
generate_window_list() {
  ...
  while IFS="[ .\.]" read -r wid ws cname cls host title; do
    ...
    *$cls*) continue ;;
    ...
    "icon")
      case "$cls" in
      Firefox) w_name=' Firefox' ;;
      Google-chrome) w_name=' Chrome' ;;
      Xfce4-terminal) w_name=' Xfce terminal' ;;
      Xfce4-taskmanager) w_name=' Task manager' ;;
      Terminator) w_name=' Terminator' ;;
      MellowPlayer3) w_name=' MellowPlayer' ;;
      Pavucontrol) w_name=' Volume control' ;;
      Xed) w_name=' Xed' ;;
      Gimp) w_name=' Gimp' ;;
      Xviewer) w_name=' Image viewer' ;;
      Thunar) w_name=' Thunar' ;;
      Nemo) w_name=' Nemo' ;;      
      File-roller) w_name=' File' ;;
      Nitrogen) w_name=' Nitrogen' ;;
      Catfish) w_name=' Catfish' ;;
      Peek) w_name='礪 Peek' ;;
      Insomnia) w_name=' Insomnia' ;;
      Pick-colour-picker) w_name=' Pick' ;;
      Gdebi-gtk) w_name=' Gdebi' ;;
      Opera) w_name=' Opera' ;;
      MenuLibre) w_name=' Menu editor' ;;
      Piper) w_name=' Piper' ;;
      Code) w_name=' VSCode' ;;
      Gnome-calculator) w_name=' Calculator' ;;
      Gnome-calendar) w_name=' Calendar' ;;
      Gnome-disks) w_name=' Disks' ;;
      Totem) w_name='辶 Videos' ;;
      teams) w_name=' Teams' ;;
      discord) w_name='ﭮ Discord' ;;
      Stremio) w_name=' Stremio' ;;
      Eog) w_name='  Pictures' ;;
      Polkit-gnome-authentication-agent-1) w_name='  Authentication' ;;      
      *) w_name="$cls" ;;
      esac
      ;;
    esac
